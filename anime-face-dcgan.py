...
paths = {
    "dataset": "dataset",
    "checkpoint": "training-checkpoints/anime-faces",
    "generated_images": "generated-images/anime-faces",
    "models": "models/anime-faces",
    "result": "result/anime-faces"
}
...
class GAN():
    ...
    def build_generator(self):
        model = keras.Sequential([
            keras.layers.Dense(
                4 * 4 * 512, input_shape=(self.latent_dims, )),
    ...
    def generate_and_save(self, epoch, test):
        ...
        for i in range(generated_images.shape[0]):
            ...
            plt.imshow(self.deprocessor(
                generated_images[i].numpy()).astype(np.uint8))
            plt.axis('off')
...
def main():
    ...
    # Set TF to limit GPU allocation for OOM errors
    ...
    generate_result(path=f"{paths['result']}/result.gif", filenames=filenames)
